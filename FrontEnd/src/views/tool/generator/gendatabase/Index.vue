<template>
  <a-card>
    ...
    <div class="btn-style">
      ...
      <a-button @click="handleExport('数据库连接')">
        <template #icon>
          <DownloadOutlined />
        </template>导出
      </a-button>
...
/** url */
const url = reactive({
  list: GenDatabaseApiUrl.INDEX_LIST,
  delete: GenDatabaseApiUrl.INDEX_DELETE,
  deleteBatch: GenDatabaseApiUrl.INDEX_DELETE_BATCH,
  exportExcel: GenDatabaseApiUrl.INDEX_EXPORT_EXCEL,
  importExcel: GenDatabaseApiUrl.INDEX_IMPORT_EXCEL,
});
...
const loadDbConnType = async () => {
  queryFormItems[0]!.options = await getDictSelect("db_conn_type");
};
loadDbConnType();
</script>