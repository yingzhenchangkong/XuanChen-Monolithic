<template>
  <a-modal v-model:open="visible" title="回收站" :width="1000">
    <template #footer>
...
import { queryParams, queryFormItems, columnsRecBin } from '../gendatabase.data';
import { GenDatabaseApiUrl } from '../gendatabase.api';

const emit = defineEmits(['childOK']);
/** url */
const url = {
  list: GenDatabaseApiUrl.RECBIN_LIST,
  delete: GenDatabaseApiUrl.RECBIN_DELETE,
  deleteBatch: GenDatabaseApiUrl.RECBIN_DELETE_BATCH,
  revert: GenDatabaseApiUrl.RECBIN_REVERT,
  revertBatch: GenDatabaseApiUrl.RECBIN_REVERT_BATCH,
}

/** 重置 */
const handleReset = () => {
  queryParams.connType = undefined;
  queryParams.connName = '';
  queryParams.host = '';
  loadData()
}
